...
durationBetween t1 t2 =
    if Time.posixToMillis t1 >= Time.posixToMillis t2 then
        ...
        let
            ...
            hours = (remainderBy (24 * 60 * 60) totalSeconds) // (60 * 60)
            minutes = (remainderBy (60 * 60) totalSeconds) // 60
    ...
    let
        days =
            if duration.days > 0 then
                String.fromInt duration.days ++ (if duration.days == 1 then " day " else " days ")
            else
                ""

        hours =
            if duration.hours > 0 then
                String.fromInt duration.hours ++ (if duration.hours == 1 then " hour " else " hours ")
            else
                ""

        minutes =
            if duration.minutes > 0 then
                String.fromInt duration.minutes ++ (if duration.minutes == 1 then " minute " else " minutes ")
            else
                ""

        seconds =
            if duration.seconds > 0 && duration.days == 0 && duration.hours == 0 && duration.minutes == 0 then
                String.fromInt duration.seconds ++ (if duration.seconds == 1 then " second " else " seconds ")
            else
                ""

        suffix = "ago"
    in
    String.trim (days ++ hours ++ minutes ++ seconds ++ suffix)
    -- ""
